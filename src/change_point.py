...
def detect_single_bayesian_changepoint(data, n_samples=3000):
    """Single change point detection (Bayesian, PyMC3)."""
    n = len(data)
    with pm.Model() as model:
        ...
        trace = pm.sample(n_samples, tune=1000, cores=2, return_inferencedata=True, progressbar=True)
    change_idx = int(trace.posterior['tau'].mean().item())
...
def detect_multiple_ruptures(data, model="l2", n_bkps=5, pen=None, show_plot=True, x=None, title="Ruptures Detected Change Points", ylabel="Value"):
    """
    Multiple change points using `ruptures`.
    - model: "l2" (mean), "rbf" (mean/variance), "l1", etc.
    - n_bkps: number of breakpoints to detect (choose based on domain knowledge or use pen=... for automatic selection)
    - pen: penalty for number of breaks (overrides n_bkps if set)
    """
    ...
    if show_plot:
        ...
        plt.ylabel(ylabel)
        plt.show()
    return bkps[:-1]  # Drop last point (end of series)

def link_change_points_to_events(bkps, dates, events_df, window_days=30):
    """
    For each change point, find closest event within ±window_days.
    Returns: List of (cp_idx, cp_date, event_row, days_diff)
    """
    links = []
...
def annotate_event_links(ax, links, y_pos, color='red'):
    """Annotate change points with event names on a plot."""
    for idx, cp_date, event, event_date, days_diff in links:
        if event is not None:
            ax.annotate(f"{event} ({event_date})", xy=(cp_date, y_pos), xycoords=('data', 'data'),
